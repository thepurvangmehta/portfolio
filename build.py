...
def _cs_gate_access_js(api_url):
    ...
    return (
        ...
        "function nextDelay(){var el=Date.now()-pollStart;"
        "return el<120000?1000:(el<600000?3000:10000);}"
        "function schedule(){if(pollDone||!curReq)return;"
        "if(pollTimer)clearTimeout(pollTimer);"
        "pollTimer=setTimeout(function(){poll(curReq);},nextDelay());}"
        "function poll(requestId){"
        "if(!requestId||pollDone||inFlight)return;"
        "curReq=requestId;if(!pollStart)pollStart=Date.now();inFlight=true;"
        "fetch(API+'/check-access?requestId='+encodeURIComponent(requestId),{cache:'no-store'})"
        ...
        "if(pollTimer){clearTimeout(pollTimer);pollTimer=null;}poll(curReq);}"
